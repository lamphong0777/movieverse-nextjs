...
import { Country, MovieFilterParams, SeoOnPage } from '@/types'
...
export const fetchMoviesByCountry = createAsyncThunk(
  ...
  async ({
    ...
    const response = await api.get(`/v1/api/quoc-gia/${slug}`, {
      params: filters,
    })
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchMoviesByCountry.fulfilled, (state, action) => {
        ...
        state.breadCrumb = Array.isArray(action.payload?.breadCrumb)

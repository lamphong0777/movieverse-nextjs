...
export const fetchMovieList = createAsyncThunk(
  ...
  async (params: MovieQueryParams, { rejectWithValue }) => {
    try {
...
export const searchMovie = createAsyncThunk(
  'movie/searchMovie',
  async (
    { keyword, page = 1, limit = 24 }: { keyword: string; page?: number; limit?: number },
    { rejectWithValue },
  ) => {
    try {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        ...
        (state, action: PayloadAction<{ items: Movie[]; type: string; totalPages: number }>) => {
...
export const { setMovieList, setLatestMovies, setMovieDetails, setLoading, setError } =
  movieSlice.actions
